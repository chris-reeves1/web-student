...
$("#customerSearch").on("input", function() {
    ...
    // this refers to the actual Element
    // .val returns the value of the input element, which is the search term.
    const term = $(this).val().toLowerCase();

    // customer is the whole Array. filter and return a new array with only
    // the customwers that match the search term.
    ...
    $("#statusMessage").text(`showing ${filtered.length} of ${customers.length}`);
});
